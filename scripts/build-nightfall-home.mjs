...
const cards = live.map(a => `<article class="hq-appcard" id="${appId(a)}" data-category="${a.group}" style="--hq-card-color:${a.colour}">
  ...
  <div class="hq-cardlinks"><a href="${route(a)}">View product page <span aria-hidden="true">↗</span></a><button type="button" data-preview="${a.key}" aria-label="Preview ${a.short}" hidden>Quick look <span aria-hidden="true">+</span></button></div>
</article>`).join('\n');
const future = upcoming.map(a => `<article id="${appId(a)}"><div><span class="hq-availability">${escape(a.stageLabel)}</span><h3>${escape(a.name)}</h3><p>${escape(a.promise)}</p></div><a class="hq-textlink" href="${route(a)}">${a.key === 'weddings' ? 'Visit the wedding site' : 'View product page'} <span aria-hidden="true">↗</span></a></article>`).join('');
