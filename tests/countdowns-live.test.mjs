...
const productFamily = home.slice(
  home.indexOf('<!-- Product Family Section -->'),
  home.indexOf('<!-- Ecosystem Section -->'),
);

assert.match(productFamily, /Let's Build Countdowns/, 'Countdowns appears in the product family');
assert.match(productFamily, /Anticipation/, 'Countdowns has the Anticipation ownership label');
...
assert.match(countdowns, /href="#features"/, 'feature CTA scrolls to the feature section');
